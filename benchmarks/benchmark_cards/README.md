...
| C1 | DG-3 | CL-2026-005 v0.4 Entry 3 (cross-method corroboration, failure-asymmetry clearance) | scope-definition (OQuPy route blocked; pending v0.4.0 pseudomode supersedure) | [C1_cross-method-pure-dephasing_v0.3.0.yaml](C1_cross-method-pure-dephasing_v0.3.0.yaml) |
| C2 | DG-3 | CL-2026-005 v0.4 Entry 4 (cross-method corroboration, failure-asymmetry clearance) | scope-definition (OQuPy route blocked; pending v0.4.0 pseudomode supersedure) | [C2_cross-method-spin-boson_v0.3.0.yaml](C2_cross-method-spin-boson_v0.3.0.yaml) |
| D1 | DG-4 | CL-2026-005 v0.4 Entry 2 (recursive-series convergence; scope-limited) | **pass** (picture-fixed Path B numerical, 2026-05-06) | [D1_failure-envelope-convergence_v0.1.2.yaml](D1_failure-envelope-convergence_v0.1.2.yaml) |
...
Card D1 v0.1.2 is the active DG-4 failure-envelope card and reached **pass** at runner version 0.1.0 on 2026-05-06 via picture-fixed Path B numerical Richardson extraction. It targets spin_boson_sigma_x, adopts the parity-aware even-order ratio `r_4(α²) = α² · <||L_4^dis||>_t / <||L_2^dis||>_t`, and freezes the reproducibility perturbation set (`upper_cutoff_factor` via numerical quadrature, `omega_c` via model-spec mutation). The Phase D run classified all 20 frozen `coupling_strength` sweep points as `convergence_failure`; no `alpha_crit` is bracketed because the first swept point already fails. Maximum baseline `r_4 ≈ 47.42`; minimum perturbed coefficient ratio `≈ 41.47` at `omega_c = 11.0`. The verdict carries the documented finite-env extraction floor caveat (~few × 1e-2 at default truncation per the 2026-05-06 σ_z thermal zero-oracle pilot); analytic Path A (Companion Sec. IV) remains preferred for machine-precision evaluation and pending. v0.1.2 supersedes v0.1.1 (whose original PASS verdict tagged `v0.5.0` was downgraded on review the same day for two HIGH-severity Path B defects + a MEDIUM audit-completeness gap, all repaired in commits d67b453..5441467). The v0.1.0 and v0.1.1 predecessors are retained with `status: superseded`. It is not analytic n=4 completion.

Cards C1/C2 v0.3.0 are retained as `scope-definition` audit artifacts for the HEOM-vs-OQuPy route, but Phase F step 1 found OQuPy 0.5.0 hard-pins `numpy<2.0`, incompatible with the numpy-2 / qutip-5.2 HEOM stack. DG-3 work plan v0.1.3 withdraws OQuPy as active and retargets the next card supersedure to C1/C2 v0.4.0 gating HEOM vs `pseudomode_reference`. The v0.3.0 cards will be atomically superseded when v0.4.0 lands.
...
| C1 | v0.1.0 | [v0.2.0](C1_cross-method-pure-dephasing_v0.2.0.yaml) | 2026-05-15 | Triple-method extension: v0.2.0 adds HEOM via `qutip.solver.heom.HEOMSolver` as a third reference method (bath-hierarchy-truncation failure-mode class) on top of the v0.1.0 baseline pair (`exact_finite_env` + `qutip_reference`). Schema bumped v0.1.2 → v0.1.4 for the new `comparison.third_method` / `gating_pair` / `heom_options` fields. Thermal-only scope; the displaced fixture is deferred to a future v0.3.0 once the HEOM module ships a displaced handler. See C1 v0.2.0 `failure_mode_log[0]`. |
| C2 | v0.1.0 | [v0.2.0](C2_cross-method-spin-boson_v0.2.0.yaml) | 2026-05-15 | σ_x sibling of C1 v0.1.0 → v0.2.0 supersedure under identical scope and rationale. See C2 v0.2.0 `failure_mode_log[0]`. |
| C1 | v0.2.0 | [v0.3.0](C1_cross-method-pure-dephasing_v0.3.0.yaml) | 2026-05-16 | Gating-pair correction: Phase D.0/D.1 recon (logbook `2026-05-16_dg-3-phase-d-recon-gating-pair-blocked`; commit `d41ff66`) proved the v0.2.0 gate `(exact_finite_env, heom_reference)` @1e-6 physically unsatisfiable (`finite-env-correlator-floor`). v0.3.0 moves the gate to `(heom_reference, oqupy_reference)` — two faithful continuous-bath solvers — under the schema v0.1.5 four-method `methods` form (Option A; exact/qutip retained non-gating). Committed `scope-definition` (oqupy_reference + N-method runner are Phase F/G). See C1 v0.3.0 `failure_mode_log[0]`. |
| C2 | v0.2.0 | [v0.3.0](C2_cross-method-spin-boson_v0.3.0.yaml) | 2026-05-16 | σ_x sibling of C1 v0.2.0 → v0.3.0 supersedure under identical scope and rationale. See C2 v0.3.0 `failure_mode_log[0]`. |
...
| D1 | v0.1.1 | [v0.1.2](D1_failure-envelope-convergence_v0.1.2.yaml) | 2026-05-06 | Same-day reviewer-side supersedure of the `v0.5.0`-tagged PASS verdict for two HIGH-severity Path B defects (picture/`Λ₀⁻¹` similarity error wrong whenever `H_S ≠ 0`; trivially-satisfied PASS predicate for `upper_cutoff_factor` perturbations) plus a MEDIUM audit-completeness gap (per-α `r_4` not persisted). v0.1.2 consumes commits d67b453 (interaction-picture transform), a908cd6 (`omega_max_factor` threaded into the finite-env builder), and 5441467 (audit-complete result JSON writer). Card structure is otherwise unchanged. |
