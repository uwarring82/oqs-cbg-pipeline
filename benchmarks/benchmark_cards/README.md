...
| B3 | DG-2 | CL-2026-005 v0.4 Entry 1.A (basis-independence) | **frozen-awaiting-run** (2026-05-01) | [B3_cross-basis-structural-identity_v0.1.0.yaml](B3_cross-basis-structural-identity_v0.1.0.yaml) |
...
Card B3 is the third DG-2 card and is also **frozen-awaiting-run**: SCHEMA.md v0.1.2 validation passes; gauge-annotation block is canonical. B3 covers the [Sail v0.5](../../docs/) §9 DG-2 universal-default structural-identity check — basis-independence of `K_from_generator` per Entry 1.A. Its three test cases reuse known-good frozen fixtures from A1 v0.1.1 and B1 v0.1.0, and assert that K computed under the matrix-unit basis equals K computed under the su(d)-generator basis ({I, σ_x, σ_y, σ_z} / √2 for d = 2). Required runner extensions: fill in the `cbg.basis.su_d_generator_basis(d)` stub for d = 2; add a basis-comparison handler factory to [`reporting/benchmark_card.py`](../../reporting/benchmark_card.py) with the per-case `comparison_basis` field (the existing AST operator parser handles all three fixture surfaces unchanged). Also a separate work step.
