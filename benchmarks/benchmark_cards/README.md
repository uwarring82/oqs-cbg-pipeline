...
| B5-conv-registry | DG-2 | CL-2026-005 v0.4 Entry 4.B.2 (eigenbasis rotation; coherent-displaced bath, σ_x coupling) | **frozen-awaiting-run** (2026-05-04) | [B5-conv-registry_v0.2.0.yaml](B5-conv-registry_v0.2.0.yaml) |
...
Card B5-conv-registry v0.2.0 is the σ_x sibling of B4-conv-registry under the same Council-cleared profile registry, with a Schrödinger-picture-corrected prediction. **frozen-awaiting-run**: SCHEMA.md v0.1.2 validation passes; gauge block canonical; the same four test_cases tag the four registry profiles. B5 covers Entry 4.B.2 (eigenbasis rotation in non-thermal bath under σ_x coupling). The corrected v0.2.0 prediction (`b_pred(t) = D̄_1(t)`, `c_pred(t) = 0`) makes B5 structurally analogous to B4 (B4: σ_z shift = 2 D̄_1; B5: σ_x coefficient = D̄_1, σ_y = 0). Per cards-first discipline, B5's runner handlers under (spin_boson_sigma_x, displaced_bath_*) are scoped after the freeze (the displaced-bath TCL recursion landed with B4's verdict commit and is reusable; only the σ_x-specific dynamical handler remains). The v0.1.0 predecessor card is retained at HEAD with `status: superseded` per SCHEMA.md §Card lifecycle (see superseded-cards table below).
