...
| `third_method` | string | optional (schema v0.1.4+) | Identifier of the third reference method (e.g. `"heom"`). When present, the card declares a **triple-method cross-method comparison**: the baseline pair (`exact_finite_env`, `qutip_reference`) plus a third method drawn from a non-overlapping failure-mode class per Sail v0.5 §5 Tier 3. Absent on baseline-pair cross-method cards (e.g. C1/C2 v0.1.0); the runner falls back to the two-method dispatch. See [DG-3 work plan v0.1.1](../../plans/dg-3-work-plan_v0.1.1.md) §2 for the selected third-method ranking. |
| `third_method_module` | string | optional (schema v0.1.4+) | Dotted Python import path of the third method's reference module (e.g. `"benchmarks.heom_reference"`). Required when `third_method` is present; absent otherwise. |
| `gating_pair` | list[string] | optional (schema v0.1.4+) | Exactly two distinct method identifiers that gate the verdict under `error_metric` and `threshold`. Under the `third_method` form they are drawn from `{exact_finite_env, qutip_reference, <third_method>_reference}`; under the `methods` form (schema v0.1.5+) both must be members of `methods`. The remaining pairwise discrepancies are recorded as auxiliary evidence in `result.notes` but do not gate. Required when either `third_method` or `methods` is present; absent otherwise. |
| `<stem>_options` | mapping | optional (schema v0.1.4+) | Per-method frozen knobs. The block name uses the method's **options stem**: the method identifier with a trailing `_reference` removed if present (`heom_reference` → `heom`, `oqupy_reference` → `oqupy`, `qutip_reference` → `qutip`, `exact_finite_env` → `exact_finite_env`), suffixed `_options`. So a `methods`-form card with method `heom_reference` freezes its knobs under `heom_options:`, and `oqupy_reference` under `oqupy_options:`. Under the `third_method` form the `third_method` value already *is* the stem (e.g. `third_method: heom` → `heom_options:`). Forwarded as `solver_options` (or equivalent) to the named reference module so the cross-method run is reproducible from the card alone, without referring to the module's internal defaults. |
| `methods` | list[string] | optional (schema v0.1.5+) | The **generalised N-method** form, superseding the single-`third_method` form for cards comparing more than three methods or gating a non-baseline pair. A list of ≥2 method identifiers run on the same frozen model spec. The two built-in methods `exact_finite_env` and `qutip_reference` need no module entry; every other listed method must have a `method_modules` entry. **Mutually exclusive with `third_method`**: a card uses either the `third_method` form (schema v0.1.4 triple) or the `methods` form (schema v0.1.5 N-method), never both. Absent ⇒ v0.1.4 pair/triple semantics unchanged. Introduced by [DG-3 work plan v0.1.2](../../plans/dg-3-work-plan_v0.1.2.md) §2.6 for the C1/C2 v0.3.0 four-method cards (gating pair `[heom_reference, oqupy_reference]`). |
| `method_modules` | mapping | optional (schema v0.1.5+) | Required when `methods` is present: maps each non-built-in method identifier in `methods` to its dotted Python import path (e.g. `heom_reference: benchmarks.heom_reference`). Built-in methods (`exact_finite_env`, `qutip_reference`) are omitted. |
...
19. **Cross-method comparison shape.** `frozen_parameters.comparison.third_method` and `frozen_parameters.comparison.methods` are each optional and **mutually exclusive** (a card carries at most one). (a) *`third_method` form (schema v0.1.4+):* when present, `third_method_module` and `gating_pair` are required; `gating_pair` is a list of exactly two distinct strings drawn from `{exact_finite_env, qutip_reference, <third_method>_reference}`; a `<third_method>_options` mapping may be present. (b) *`methods` form (schema v0.1.5+):* when present, `methods` is a list of ≥2 distinct method identifiers; `method_modules` is required and maps every method **not** in the built-in set `{exact_finite_env, qutip_reference}` to an importable dotted module path; `gating_pair` is required and is exactly two distinct identifiers, both members of `methods`; per-method frozen knobs use the **options-stem convention** — for method `m`, the block is `<stem>_options` where `<stem>` is `m` with a trailing `_reference` removed if present (`heom_reference` → `heom_options`, `oqupy_reference` → `oqupy_options`, `qutip_reference` → `qutip_options`, `exact_finite_env` → `exact_finite_env_options`); every `*_options` block present must resolve to the stem of some member of `methods`. (c) *Neither present:* baseline two-method pair (`exact_finite_env` vs `qutip_reference`), unchanged from schema ≤ v0.1.4. Backward compatibility: cards authored without `methods` validate exactly as under v0.1.4.
...
This schema is itself versioned. The current version is `v0.1.5` (drafted 2026-05-16; surfaced by [DG-3 work plan v0.1.2](../../plans/dg-3-work-plan_v0.1.2.md) §2.6 Phase E drafting of Cards C1 / C2 v0.3.0; see Revision history below). The v0.1.2 baseline (drafted 2026-04-30, [DG-1 work plan v0.1.2](../../plans/dg-1-work-plan_v0.1.2.md) Phase A/Phase B boundary) remains the schema under which the DG-1 cards (A1, A3, A4) and the DG-2 cards (B1–B5) were authored, and those cards continue to validate unchanged under v0.1.5 per the supersedure-stability rule below.
...
- **v0.1.3 (2026-05-05, superseded by v0.1.4).** Added `frozen_parameters.sweep:` block for DG-4 failure-envelope and convergence-study cards (Rule 17). Added `scope-definition` status value for cards whose preconditions are not yet met (e.g. model API is stubbed, competing-framework reference missing). Expanded Rule 2 (status enum), Rule 3 (empty-result precondition), and added Rule 18 (scope-definition notes requirement). Surfaced by Phase B drafting of Cards D1 (coupling-strength sweep) and E1 (Fano-Anderson scope definition): D1 needed a machine-readable sweep specification, and E1 needed a status that distinguished "intentionally not runnable" from "frozen awaiting implementation". MINOR bump because new fields, new status value, and new validation rules are added; non-breaking because v0.1.2 cards (no sweep block, status `frozen-awaiting-run`) continue to validate unchanged under v0.1.3 (the additions are additive). Same cards-first-surfaces-schema-issues pattern as prior bumps.
- **v0.1.4 (2026-05-15, superseded by v0.1.5).** Added optional triple-method cross-method fields under `frozen_parameters.comparison`: `third_method`, `third_method_module`, `gating_pair`, and per-method `<method>_options` mappings. Added Rule 19: when `third_method` is present, `third_method_module` and `gating_pair` are required, `gating_pair` is a list of exactly two strings drawn from the set of methods participating in the run, and a corresponding `<third_method>_options` mapping may be present to freeze method-specific knobs. Surfaced by [DG-3 work plan v0.1.1](../../plans/dg-3-work-plan_v0.1.1.md) Phase C drafting of Cards C1 v0.2.0 and C2 v0.2.0, which extend the C1/C2 v0.1.0 baseline pair (`exact_finite_env` + `qutip_reference`) with HEOM via QuTiP 5's in-tree `qutip.solver.heom.HEOMSolver` as the third reference method. MINOR bump because new optional fields and a new validation rule are added; non-breaking because v0.1.3 cards (no `third_method` field) continue to validate unchanged under v0.1.4 (the additions are additive). Same cards-first-surfaces-schema-issues pattern as prior bumps.
- **v0.1.5 (2026-05-16, this revision).** Added the generalised **N-method** cross-method form under `frozen_parameters.comparison`: `methods` (list of ≥2 method identifiers) and `method_modules` (dotted import path per non-built-in method). Generalised Rule 19 to a three-branch shape: `third_method` form (v0.1.4) and `methods` form (v0.1.5) are mutually exclusive; under the `methods` form `gating_pair` ⊂ `methods`, every non-built-in method has a `method_modules` entry, and per-method frozen knobs follow the **options-stem convention** (`<m without trailing _reference>_options`, e.g. `heom_reference` → `heom_options`). Surfaced by [DG-3 work plan v0.1.2](../../plans/dg-3-work-plan_v0.1.2.md) §2.6 Phase E: the Phase D.0/D.1 reconnaissance (logbook `2026-05-16_dg-3-phase-d-recon-gating-pair-blocked`) found the C1/C2 v0.2.0 triple-method gating pair `(exact_finite_env, heom_reference)` physically unsatisfiable (`finite-env-correlator-floor`), requiring a four-method C1/C2 v0.3.0 card that gates `[heom_reference, oqupy_reference]` while retaining `exact_finite_env`/`qutip_reference` as non-gating auxiliary trajectories — a shape the singular `third_method` field cannot express. MINOR bump because new optional fields and a generalised (still backward-compatible) validation rule are added; non-breaking because v0.1.4 cards (no `methods` field) continue to validate unchanged under v0.1.5 (the `methods` form is additive and mutually exclusive with `third_method`). Same cards-first-surfaces-schema-issues pattern as prior bumps.
