...
| `gating_pair` | list[string] | optional (schema v0.1.4+) | Exactly two method identifiers — drawn from `{exact_finite_env, qutip_reference, <third_method>_reference}` — that gate the verdict under `error_metric` and `threshold`. The remaining pairwise discrepancies are recorded as auxiliary evidence in `result.notes` but do not gate. Required when `third_method` is present; absent otherwise. |
| `<method>_options` | mapping | optional (schema v0.1.4+) | Per-method frozen knobs, keyed by method identifier (e.g. `heom_options:` for `third_method: heom`). Forwarded as `solver_options` (or equivalent) to the named reference module so that the cross-method run is reproducible from the card alone, without referring to the module's internal defaults. |
...
19. **`frozen_parameters.comparison.third_method` is optional. When present, `frozen_parameters.comparison.third_method_module` and `frozen_parameters.comparison.gating_pair` are required; `gating_pair` is a list of exactly two strings drawn from the set of methods participating in the run (canonically `exact_finite_env`, `qutip_reference`, and `<third_method>_reference`). A corresponding `<third_method>_options` mapping under `frozen_parameters.comparison` may be present to freeze method-specific knobs.** Schema v0.1.4+.
...
This schema is itself versioned. The current version is `v0.1.4` (drafted 2026-05-15; surfaced by [DG-3 work plan v0.1.1](../../plans/dg-3-work-plan_v0.1.1.md) Phase C drafting of Cards C1 / C2 v0.2.0; see Revision history below). The v0.1.2 baseline (drafted 2026-04-30, [DG-1 work plan v0.1.2](../../plans/dg-1-work-plan_v0.1.2.md) Phase A/Phase B boundary) remains the schema under which the DG-1 cards (A1, A3, A4) and the DG-2 cards (B1–B5) were authored, and those cards continue to validate unchanged under v0.1.4 per the supersedure-stability rule below.
