...
| DG-4 | RUNNER-COMPLETE; v0.1.1 PASS verdict (2026-05-06, tag `v0.5.0`) **superseded on review** the same day; v0.1.2 supersedure pending | Card [D1 v0.1.1](https://github.com/uwarring82/oqs-cbg-pipeline/blob/main/benchmarks/benchmark_cards/D1_failure-envelope-convergence_v0.1.1.yaml) — spin-boson σ_x thermal, parity-aware ratio `r_4 = ⟨‖L_4^dis‖⟩_t / ⟨‖L_2^dis‖⟩_t`. Verdict superseded for two HIGH-severity defects: Path B order-4 extraction in the wrong picture for `H_S ≠ 0`, and PASS predicate trivially satisfied for `upper_cutoff_factor ∈ {20, 40}` because Path B does not consume the knob. v0.1.2 supersedure work: repair Path B picture handling, operational `omega_max_factor`, audit-complete result JSON, re-run. See [supersedure log](https://github.com/uwarring82/oqs-cbg-pipeline/blob/main/logbook/2026-05-06_dg-4-pass-path-b-superseded.md). |
