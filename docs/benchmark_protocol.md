...
|---|---|---|
| `exact_finite_env.py` + `qutip_reference.py` + `heom_reference.py` | COMPLETE for the C1/C2 v0.2.0 thermal fixtures (triple-method dispatch); baseline pair runner-reachable for the C1/C2 v0.1.0 displaced fixtures retained for reproduction | NOT CLEARED — Phase D verdict on C1/C2 v0.2.0 is **blocked** (cause `finite-env-correlator-floor`; Phase D.0/D.1 recon 2026-05-16). The three method classes are non-overlapping per §2, but the frozen gating pair `(exact_finite_env, heom_reference)` @1e-6 is physically unsatisfiable: `exact_finite_env`'s discrete bath correlator converges only ≈ O(1/n_modes) vs the continuous bath HEOM solves. |
| `heom_reference.py` + planned `pseudomode_reference.py` | HEOM implemented; pseudomode module and C1/C2 v0.4.0 cards pending under DG-3 work plan v0.1.3. | NOT CLEARED — active route after OQuPy/TEMPO was blocked by OQuPy 0.5.0's `numpy<2.0` dependency pin. See logbook [`2026-05-16_dg-3-oqupy-numpy2-dependency-blocked`](../logbook/2026-05-16_dg-3-oqupy-numpy2-dependency-blocked.md). |
...
|---|---|---|---|
| C1 v0.3.0 | pure_dephasing | exact_finite_env + qutip_reference + heom_reference + oqupy_reference (gating pair: `[heom_reference, oqupy_reference]`) | scope-definition; OQuPy route blocked by `numpy<2.0`; to be superseded by future C1 v0.4.0 pseudomode route |
| C2 v0.3.0 | spin_boson_sigma_x | exact_finite_env + qutip_reference + heom_reference + oqupy_reference (same gating pair) | scope-definition; OQuPy route blocked by `numpy<2.0`; to be superseded by future C2 v0.4.0 pseudomode route |

Superseded predecessors (retained for audit and reproduction of the baseline-pair runner reachability):

| Card | Model | Methods compared | Status |
|---|---|---|---|
| C1 v0.2.0 | pure_dephasing | exact_finite_env + qutip_reference + heom_reference | superseded by C1 v0.3.0; Phase D.0/D.1 proved gating pair `[exact_finite_env, heom_reference]` physically unsatisfiable (`finite-env-correlator-floor`) |
| C2 v0.2.0 | spin_boson_sigma_x | exact_finite_env + qutip_reference + heom_reference | superseded by C2 v0.3.0; same finite-env gating-pair blocker |
