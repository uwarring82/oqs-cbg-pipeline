...
**Last updated:** 2026-05-05 (DG-3 Phase C C1 displaced handler wired; see `logbook/2026-05-05_dg-3-c1-displaced-handler-wired.md`)
...
| DG-3 | Cross-method validation (≥2 methods, non-overlapping failure modes) | **PARTIAL IMPLEMENTATION — full C1 runner wired (thermal + displaced delta-omega_c); C2 awaiting handlers** (2026-05-05) | Cards [C1 v0.1.0](../benchmarks/benchmark_cards/C1_cross-method-pure-dephasing_v0.1.0.yaml), [C2 v0.1.0](../benchmarks/benchmark_cards/C2_cross-method-spin-boson_v0.1.0.yaml) (frozen-awaiting-run); DG-3 work plan [v0.1.0](../plans/dg-3-work-plan_v0.1.0.md) (draft); `reporting.benchmark_card._run_cross_method`; `tests/test_benchmark_card.py` DG-3 runner tests. | Baseline pair (`exact_finite_env.py` + `qutip_reference.py`) is implemented and runner-wired for both C1 fixtures: `thermal_bath_cross_method` runs to FAIL at `inter_method_relative_frobenius ≈ 0.293 > 1.0e-6`; `displaced_bath_delta_omega_c_cross_method` runs to FAIL at `≈ 0.309 > 1.0e-6`. Both FAILs are consistent with finite-bath recurrences vs Markov decay — the cross-method test working as designed. C1 verdict block is now reachable end-to-end; C2 handlers (`thermal_bath_cross_method` and `displaced_bath_delta_omega_c_cross_method` for `spin_boson_sigma_x`) remain explicit deferred `NotImplementedError` routes. No DG-3 PASS or failure-asymmetry clearance is claimed. |
...
|---|---|---|
| `exact_finite_env.py` + `qutip_reference.py` | PARTIAL — full C1 runner wired (thermal + displaced delta-omega_c); C2 handlers deferred | NOT CLEARED at DG level (method classes are distinct, but C2 coverage is incomplete and both methods may share finite-truncation/solver assumptions) |
