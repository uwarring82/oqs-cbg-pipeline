...
| DG-3 | Cross-method validation (≥2 methods, non-overlapping failure modes) | **SCOPED — frozen cards awaiting implementation** (2026-05-05) | Cards [C1 v0.1.0](../benchmarks/benchmark_cards/C1_cross-method-pure-dephasing_v0.1.0.yaml), [C2 v0.1.0](../benchmarks/benchmark_cards/C2_cross-method-spin-boson_v0.1.0.yaml) (frozen-awaiting-run); DG-3 work plan [v0.1.0](../plans/dg-3-work-plan_v0.1.0.md) (draft). | Two cross-method benchmark cards frozen. Baseline pair (`exact_finite_env.py` + `qutip_reference.py`) remains scaffolded. Implementation-ready pass is the target; failure-asymmetry clearance is aspirational (pending third method from non-overlapping class). |
...
1. **DG-3 implementation-ready pass**: implement `exact_finite_env.py` and `qutip_reference.py`, wire cross-method comparison into the runner, and run Cards C1 and C2.
