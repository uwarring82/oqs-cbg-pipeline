...
**Last updated:** 2026-05-05 (DG-3 C2 thermal handler wired; see `logbook/2026-05-05_dg-3-c2-thermal-handler-wired.md`)
...
| DG-3 | Cross-method validation (≥2 methods, non-overlapping failure modes) | **PARTIAL IMPLEMENTATION — full C1 wired + C2 thermal wired; C2 displaced awaiting handler** (2026-05-05) | Cards [C1 v0.1.0](../benchmarks/benchmark_cards/C1_cross-method-pure-dephasing_v0.1.0.yaml), [C2 v0.1.0](../benchmarks/benchmark_cards/C2_cross-method-spin-boson_v0.1.0.yaml) (frozen-awaiting-run); DG-3 work plan [v0.1.0](../plans/dg-3-work-plan_v0.1.0.md) (draft); `reporting.benchmark_card._run_cross_method`; `tests/test_benchmark_card.py` DG-3 runner tests. | Baseline pair (`exact_finite_env.py` + `qutip_reference.py`) is implemented and runner-wired for: C1 thermal (`error ≈ 0.293`), C1 displaced delta-omega_c (`≈ 0.309`), and C2 thermal (`≈ 0.538`). All three run to FAIL at the frozen 1.0e-6 threshold; the σ_x case shows a larger Markov-vs-exact gap because σ_x couples to ±ω_S transitions where finite-bath truncation has stronger effect than σ_z's S(0) channel. The C2 displaced fixture (`displaced_bath_delta_omega_c_cross_method` for `spin_boson_sigma_x`) remains an explicit deferred `NotImplementedError` route — the only remaining gate before the full frozen C1+C2 cards reach end-to-end runner reachability. No DG-3 PASS or failure-asymmetry clearance is claimed. |
...
|---|---|---|
| `exact_finite_env.py` + `qutip_reference.py` | PARTIAL — full C1 runner wired (thermal + displaced delta-omega_c); C2 thermal wired; C2 displaced deferred | NOT CLEARED at DG level (method classes are distinct, but C2 displaced is incomplete and both methods may share finite-truncation/solver assumptions) |
