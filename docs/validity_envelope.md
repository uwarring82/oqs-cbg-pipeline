...
**Last updated:** 2026-05-05 (DG-3 C2 displaced handler wired — full C1+C2 coverage; see `logbook/2026-05-05_dg-3-c2-displaced-handler-wired.md`)
...
| DG-4 | Failure envelope (≥1 reproducible, cause-labelled failure regime) | **SCOPED — frozen card awaiting implementation** (2026-05-05) | Card [D1 v0.1.0](../benchmarks/benchmark_cards/D1_failure-envelope-convergence_v0.1.0.yaml) (frozen-awaiting-run). | One failure-envelope card frozen: coupling-strength sweep in pure-dephasing model to identify perturbative convergence breakdown. Cause-label taxonomy defined in `docs/benchmark_protocol.md` §2. Runner does not yet support parameter sweeps. |
| DG-5 | Thermodynamic discriminant (≥1 distinguishable observable in solvable model) | **SCOPED — scope-definition card frozen** (2026-05-05) | Card [E1 v0.1.0](../benchmarks/benchmark_cards/E1_thermodynamic-discriminant-fano-anderson_v0.1.0.yaml) (frozen-awaiting-run). | Scope-definition card frozen for Fano-Anderson model. Preconditions not yet met: no callable model API, no competing-framework reference, no fermionic-bath support in cumulants. DG-5 outputs route via fresh Council deliberation per Sail §9. |
...
At the current status (2026-05-04, DG-1 PASS + DG-2 structural sub-claims PASS under Council-cleared registry):
...
- Cross-method validation PASS claims (DG-3 territory): only the C1 thermal fixture is implemented and runner-wired, and it returns FAIL at the frozen threshold; the full frozen C1/C2 cards have not reached verdict blocks.
...
1. **DG-3 implementation-ready pass**: complete the deferred C1 displaced and C2 handlers, then run full Cards C1 and C2 to verdict.
2. **DG-4 first failure regime**: implement parameter-sweep support in the runner, run Card D1, and identify a reproducible, cause-labelled convergence breakdown.
...
4. **Literal K_2–K_4 numerical recursion**: unblocks the residual "scope-limited" qualifier on Entry 2, now that structural identities (Entry 1.A, 1.B.3, 1.D) are verified.
