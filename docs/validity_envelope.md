...
**Last updated:** 2026-05-05 (DG-3 Phase C partial implementation: C1 thermal cross-method runner wired; see `logbook/2026-05-05_dg-3-phase-c-runner-wired.md`)
...
| DG-3 | Cross-method validation (≥2 methods, non-overlapping failure modes) | **PARTIAL IMPLEMENTATION — C1 thermal runner wired; full cards awaiting deferred handlers** (2026-05-05) | Cards [C1 v0.1.0](../benchmarks/benchmark_cards/C1_cross-method-pure-dephasing_v0.1.0.yaml), [C2 v0.1.0](../benchmarks/benchmark_cards/C2_cross-method-spin-boson_v0.1.0.yaml) (frozen-awaiting-run); DG-3 work plan [v0.1.0](../plans/dg-3-work-plan_v0.1.0.md) (draft); `reporting.benchmark_card._run_cross_method`; `tests/test_benchmark_card.py` DG-3 runner tests. | Baseline pair (`exact_finite_env.py` + `qutip_reference.py`) is implemented and runner-wired for C1 `thermal_bath_cross_method`. That case runs cleanly and returns FAIL at the frozen threshold (`inter_method_relative_frobenius ≈ 0.548 > 1.0e-6`), consistent with finite-bath recurrences vs Markovian decay. Full C1 and C2 verdict blocks are not yet admissible: C1 displaced and C2 handlers remain explicit deferred `NotImplementedError` routes. No DG-3 PASS or failure-asymmetry clearance is claimed. |
...
|---|---|---|
| `exact_finite_env.py` + `qutip_reference.py` | PARTIAL — C1 thermal fixture implemented and runner-wired; full frozen C1/C2 coverage incomplete | NOT CLEARED at DG level (method classes are distinct, but full card coverage is incomplete and both methods may share finite-truncation/solver assumptions) |
