...
| [DG-3 Work Plan](dg-3-work-plan_v0.1.1.md) | v0.1.1 | draft | Tier-2.A third-method selection plan: HEOM (in-tree `qutip.solver.heom.HEOMSolver`) selected; TEMPO (OQuPy) fallback; pseudomode noted. Phase A: decision-record (this revision) + `qutip>=5.2` floor bump; Phases B–D (`benchmarks/heom_reference.py` scaffolding, C1/C2 v0.2.0 card revision, run + verdict) pending. | Sail v0.5 §9 (DG-3 failure-asymmetry clearance per §5 Tier 3). Baseline-pair runner wiring landed under v0.1.0 (validity envelope DG-3 row 2026-05-05); v0.1.1 covers the remaining third-method blocker. Linked to the 2026-05-13 DG-4 routing-fork recommendation as the two-for-one DG-3 PASS + DG-4 Phase E Path B replacement move. |
