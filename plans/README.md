...
| [DG-3 Work Plan](dg-3-work-plan_v0.1.1.md) | v0.1.1 | draft; Phases A–C landed | Tier-2.A third-method selection plan: HEOM (in-tree `qutip.solver.heom.HEOMSolver`) selected; TEMPO (OQuPy) fallback; pseudomode noted. **Phase A** decision-record + `qutip>=5.2` floor bump landed 2026-05-15 (commit `672db39`). **Phase B** [`benchmarks/heom_reference.py`](../benchmarks/heom_reference.py) module + 12 smoke tests landed (commit `f30c627`). **Phase C** SCHEMA.md v0.1.3 → v0.1.4 (Rule 19 for triple-method fields), C1/C2 v0.2.0 thermal-only triple-method cards frozen, `_run_cross_method` extended with the `third_method` branch + `_CROSS_METHOD_TRIPLE_HANDLERS` registry, C1/C2 v0.1.0 atomic supersedure. **Phase D** verdict **blocked** (Phase D.0/D.1 recon 2026-05-16, logbook [`2026-05-16_dg-3-phase-d-recon-gating-pair-blocked`](../logbook/2026-05-16_dg-3-phase-d-recon-gating-pair-blocked.md)): the C1/C2 v0.2.0 frozen gating pair `(exact_finite_env, heom_reference)` @1e-6 is physically unsatisfiable (cause `finite-env-correlator-floor`; `exact_finite_env`'s discrete correlator converges only ≈ O(1/n_modes), HEOM internally converged, no convention bug). v0.2.0 cards remain frozen and unmutated. Recommended route (steward decision pending): C1/C2 **v0.3.0** supersedure gating HEOM vs **TEMPO (OQuPy)** per §2.3 + new `benchmarks/oqupy_reference.py`. | Sail v0.5 §9 (DG-3 failure-asymmetry clearance per §5 Tier 3). Baseline-pair runner wiring landed under v0.1.0 (validity envelope DG-3 row 2026-05-05); v0.1.1 closes the third-method blocker. Linked to the 2026-05-13 DG-4 routing-fork recommendation as the two-for-one DG-3 PASS + DG-4 Phase E Path B replacement move. |
