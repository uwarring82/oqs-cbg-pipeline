...
| [DG-3 Work Plan](dg-3-work-plan_v0.1.3.md) | v0.1.3 | draft; Phases A–E landed; OQuPy route blocked → pseudomode selected; Phase F.0 remediation included | HEOM-vs-**pseudomode** gating-pair plan, superseding v0.1.2 after Phase F step 1 found OQuPy 0.5.0 (latest) hard-pins `numpy<2.0` — mutually exclusive with this repo's validated numpy-2 / qutip-5.2 HEOM stack (logbook [`2026-05-16_dg-3-oqupy-numpy2-dependency-blocked`](../logbook/2026-05-16_dg-3-oqupy-numpy2-dependency-blocked.md)). **Phases A–C** HEOM track landed (`672db39`, `f30c627`, `0b750b2`); **D.0/D.1 recon** (`d41ff66`) found the v0.2.0 `(exact_finite_env, heom_reference)` @1e-6 gate unsatisfiable (`finite-env-correlator-floor`); **Phase E** (`1b4c21d`) landed schema v0.1.5 N-method form + C1/C2 v0.3.0 `scope-definition` cards + the now-blocked `oqupy` dep. v0.1.3 **withdraws the OQuPy route** (dormant unless upstream numpy-2 support lands), **promotes pseudomode** (QuTiP `mesolve`; no new dependency; numpy-2-native; non-overlapping auxiliary-system class). Card supersedure retargeted to **C1/C2 v0.4.0** (gating `[heom_reference, pseudomode_reference]`); schema v0.1.5 scaffolding retained (no schema bump). **Phase F.0 included**: remove the uninstallable `oqupy>=0.5,<0.6` hard dep from `pyproject.toml` and update README dependency prose. Next: F (`benchmarks/pseudomode_reference.py`), G (v0.4.0 cards + N-method runner), H (verdict). | Sail v0.5 §9 (DG-3 failure-asymmetry clearance per §5 Tier 3). |
...
| [DG-3 Work Plan v0.1.2](dg-3-work-plan_v0.1.2.md) | v0.1.2 | [v0.1.3](dg-3-work-plan_v0.1.3.md) | 2026-05-16 |
