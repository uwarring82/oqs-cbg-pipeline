...
| [DG-4 Phase C physics-oracles card](colla-breuer-gasbarri-2025_companion-sec-iv_l4_phase-c-physics-oracles-card_v0.1.2.md) | v0.1.2 | frozen-pre-implementation (2026-05-13; current) | Supersedes v0.1.1 by refining the σ_z gate into a **two-part oracle**: (Part A) commuting-case Feynman-Vernon exact-zero via a §3.2 API guard (`if [H_S, A] = 0: return zero callable`) at `atol=1e-12`; (Part B) literal-quadrature convergence diagnostic on a refinement table (n_pts ∈ {11, 21, 41, 81}), monotonic-decrease assertion only, NOT a gate. The v0.1.1 implementation experiment showed O(h^1) convergence with ~1e-2 residual at N=11, insufficient for 1e-10 via trapezoid refinement; the commuting case is enforced analytically instead. σ_x signal, gauge/sign, parity oracles carried forward unchanged. §3a θ-aware integration discipline still pinned for the non-commuting path. |
...
*Last updated: 2026-05-13 (Companion Sec. IV L4 transcription released at v0.1.1; DG-4 Phase B small-grid card v0.1.0 frozen and helper landed at commit `becccf9`; DG-4 Phase C physics-oracles card v0.1.2 frozen pre-implementation with two-part σ_z gate — Part A commuting-case Feynman-Vernon exact-zero via API guard, Part B non-gating O(h^1) convergence diagnostic. v0.1.0 and v0.1.1 retained as superseded predecessors documenting the implementation evolution). CC-BY-4.0 (see ../LICENSE-docs).*
