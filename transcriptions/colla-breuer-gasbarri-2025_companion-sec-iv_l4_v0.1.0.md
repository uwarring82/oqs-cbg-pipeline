...
> Bibliographic record pinned; §§3–4 equation transcription drafted from
> the Version-of-Record PDF; §2 sign-convention rows populated. Row 2.8
> is `Draft-complete` on the B.0/B.1 vs. Companion `D̄` distinction
> (component (a)) but carries an `Open` sub-component (b): the repo-side
> flattening convention `times = tau_args + reversed(s_args)` is **not**
> proven equal to Companion Eq. (15)'s operator chain at mixed `(k, n-k)`;
> at `n=2`, `k=1` it differs by complex conjugation (see row 2.3). The
> repo flattening must be treated as a conversion hypothesis pending
> derivation, not a source identity. **No implementation work (Phase B
> onward) should begin until:** (i) the repo-flattening ↔ Eq. (15)
> conversion rule for mixed `(k, n-k)` at `n=4` is derived and recorded,
> (ii) the §2.8 grid-verification is signed off, and (iii) the §10
> sign-off block is countersigned.
...
comments. Promotion to released state requires all three of the
following:

1. **Row 2.8 component (b) — repo-flattening ↔ Eq. (15) conversion rule
   for mixed `(k, n-k)` at `n=4`** — derived and recorded.
2. **Row 2.3 — repo `D_bar_2(times)[0,1] = ⟨B(times[0]) B(times[1])⟩`
   vs. Companion Eq. (15)'s `⟨B(s_1) B(τ_1)⟩` at `k=1`** — explicit
   conjugation conversion (or its negation) recorded in §2.3 with the
   generalisation rule for `n ≥ 3`.
3. **§10 sign-off block** — countersigned by the steward, with §2.8
...
| 2.3 | Sign of bath two-point function `C(t, s)` | Eq. (15) verbatim: `D(τ_1^k, s_1^{n−k}) = Tr_E{ B^R(s_1^{n−k}) ∘ B^L(τ_1^k) [ρ_E] } θ_{τ_1^k} θ_{s_1^{n−k}}`. For the simplest case `n=2`, `k=1`: `D(τ_1, s_1) = ⟨B(s_1) B(τ_1)⟩ θ_{τ_1} θ_{s_1}` (right-acting B applied last under the trace, then left-acting B). Note the **Hermiticity** identity Eq. (18): `D*(τ_1^k, s_1^{n−k}) = D(s_1^{n−k}, τ_1^k)`. | Repository n=2 stores `D̄_2(t, s) = C(t − s)` (stationary baseline). `cbg.cumulants.D_bar_2(times)[0, 1]` evaluates to `⟨B(times[0]) B(times[1])⟩` (see [cbg/cumulants.py D_bar_2 path](../cbg/cumulants.py) + scalar leaf [`_D_bar_scalar_from_flat_times`](../cbg/cumulants.py) and the mixed-n=2 witness [tests/test_cumulants.py:test_D_bar_mixed_n2_thermal_matches_pairwise_correlator](../tests/test_cumulants.py)); under the B.0 flattening `times = tau_args + reversed(s_args)`, this returns `⟨B(τ_1) B(s_1)⟩`. | **Repo conversion hypothesis pending derivation — not source-identity.** Eq. (15) at `n=2`, `k=1` evaluates to `⟨B(s_1) B(τ_1)⟩`; the repo flatten returns `⟨B(τ_1) B(s_1)⟩`. For Hermitian `B` on a stationary state these differ by complex conjugation (`⟨B(s) B(τ)⟩ = ⟨B(τ) B(s)⟩*` for stationary bosonic correlators with reflection-symmetric spectral density), so the two scalars are **conjugates** and **not equal in general**. The conversion is therefore a `D_repo ↔ D_companion = D_repo*` relation at `k=1`, plausibly generalising to a chain-reversal-plus-conjugation rule for mixed `(k, n-k)` at higher `n`. The all-left case (`s_args = ()`) is invariant under this conversion and is the regime in which the repo's existing n=2 route was validated. **Status: hypothesis; steward must derive the explicit conversion rule for mixed `(k, n-k)` at `n=4` before code lands.** | **Open — conversion hypothesis pending derivation.** Was First-pass-Pinned in the 2026-05-12 first pass; demoted in fix-pass #3 after the user flagged the §2.3/§2.8 internal inconsistency. |
...
| 2.8 | Ordering of bath operators in 4-point correlator (Wick contractions) | **Sec. IV does NOT explicitly write Wick's theorem for the 4-point bath correlator.** Sec. IV.D computes the `n = 4` cumulants (Eqs. 69–73) using the recursion Eq. (27), **not** an explicit Wick split. The 4-point bath quantities `D(τ_1^4)`, `D(τ_1^3, s_1)`, `D(τ_1^2, s_1^2)`, `D(τ_1, s_1^3)`, `D(s_1^4)` are treated as **given inputs** (n-point environmental correlators with the time ordering inherited from Eq. (15)). For a **thermal Gaussian** bath, Wick's theorem applies independently — but this is **not** a Companion equation; it is a model-level fact about the bath. | **n=4 repository routes: B.0 raw correlator path is the repo's mixed-order Wick input; B.1 standard-cumulant path must NOT be reused for Companion n=4.** `cbg.bath_correlations.n_point_ordered` (DG-4 Phase B.0) evaluates the raw thermal Gaussian 4-point correlator by Wick factorisation into the existing two-point `C(t_i, t_j)` with the three bosonic pairings at `+1` coefficient (see [`tests/test_bath_correlations.py:test_n_point_ordered_thermal_n4_wick_all_left`](../tests/test_bath_correlations.py)). The repo flattening convention `times = tau_args + reversed(s_args)` (pinned by [`n_point_ordered`](../cbg/bath_correlations.py)) is the repo's own mixed-order indexing; **its equivalence to Companion Eq. (15)'s `Tr_E{B^R(s_1^{n−k}) B^L(τ_1^k) ρ_E}` chain is a repo conversion hypothesis pending derivation, not an established source identity** (see row 2.3 for the n=2 case where the same hypothesis already requires a conjugation conversion at `k=1`). Independently, `cbg.cumulants._joint_cumulant_from_raw_moments` (B.1) computes standard statistical cumulants via set partitions, which vanish for thermal Gaussian at n ≥ 3; the Companion's `D̄` at n=4 is **not** the standard statistical cumulant. Companion Eqs. (69)–(73) use `Ḋ` (time-derivative with boundary delta, Eq. 22) and explicit subtractions of lower-order raw correlators, giving non-zero `D̄` even for Gaussian baths. | **Conversion rule has two distinct components.** (a) **Companion-side `D̄` recursion is distinct from the repository's standard-cumulant path.** Phase B must implement Companion Eqs. (69)–(73) directly, reusing the B.0 raw-correlator leaves for the raw `D` entries but **not** reusing `_joint_cumulant_from_raw_moments` for the n=4 `D̄`. The explicit subtraction formulas encode the boundary-time delta structure (`Ḋ`) that the standard set-partition cumulant formula omits. (b) **Repo flattening ↔ Eq. (15) operator chain is a separate, undischarged hypothesis.** Specifically: whether the repo's mixed `(tau_args, s_args)` indexing — once flattened as `times = tau_args + reversed(s_args)` and fed through `n_point_ordered` — agrees with Eq. (15) literally, or only after a chain-reversal-plus-conjugation conversion analogous to row 2.3's `k=1` case, is an open derivation. The §4.4 θ-aware procedural rule governs how the raw 4-point and disconnected subtractions are combined inside their distinct θ-windows **independently** of this open hypothesis. | **Draft-complete on component (a); component (b) Open as a repo-side conversion hypothesis pending derivation.** Component (a) (B.0 ≠ B.1 vs. Companion `D̄`) is the more consequential finding for Phase B: B.1 reuse would silently zero out `L_4` for thermal Gaussian. Component (b) (flattening ↔ Eq. (15)) determines whether the Phase B implementation must thread a chain-reversal-conjugation onto the B.0 leaves; this must be derived explicitly before Phase B code lands. Steward must (i) derive the repo-flattening ↔ Eq. (15) conversion rule at `n = 4` mixed-`(k, 4-k)`, (ii) verify the explicit Eq. (69)–(73) implementation on a small test grid, and (iii) verify against the existing n=4 test witnesses [`test_D_bar_thermal_n4_all_left_vanishes_by_gaussianity`](../tests/test_cumulants.py) and [`test_D_bar_thermal_n4_mixed_left_right_vanishes_by_gaussianity`](../tests/test_cumulants.py) — these confirm that the **standard-cumulant** path returns zero (as expected), not that it matches the Companion `D̄`. |
...
2026-05-12, §2.8 fill-in 2026-05-12, fix-pass #3 2026-05-12). Drafted
2026-05-11 as the Phase A artifact for `dg-4-work-plan_v0.1.5`. The
2026-05-12 first-pass populated §0 bibliographic block from the APS
Version-of-Record, filled §2 rows where the paper is unambiguous,
expanded §3 to a complete symbol table for Sec. IV, transcribed
Eqs. (28), (43), (45)–(49), (69)–(78) verbatim into §4, recorded the
Λ-inversion reconciliation as Case B in §5, and strengthened the §7.1
σ_z oracle check. Fix-pass #1 corrected the §7.1 dephasing exactness
citation to Letter Eq. (21). Fix-pass #2 replaced an early §4.4
pointwise pairing-count rule with a θ-aware procedural rule, split the
§0 provenance row to match actual §4 contents, and dropped the §10
arXiv-version-pinned claim so the VoR DOI is the sole controlling
source. The §2.8 fill-in moved row 2.8 from Open to Draft-complete on
the B.0/B.1 vs. Companion `D̄` distinction. **Fix-pass #3 (after a
local-code re-review)** demoted §2.3 (and §2.8's component (b)) from
"matches Eq. (15)" to a repo conversion hypothesis pending derivation:
at `n=2`, `k=1` the repo flatten returns `⟨B(τ_1) B(s_1)⟩` while
Eq. (15) evaluates to `⟨B(s_1) B(τ_1)⟩` — these differ by complex
conjugation for stationary Hermitian-B baths, so the flattening ↔
Eq. (15) equivalence is **not** a source identity in general and must
be derived for mixed `(k, n-k)` at `n=4` before Phase B code lands. The
§0 pre-release marker now enumerates the three explicit release
preconditions (2.8(b) conversion derivation, 2.3 generalisation, §10
sign-off); the head-of-file status note matches. The B.1 standard-
cumulant test witnesses are
...
not begin until the three §0 preconditions are met. CC-BY-4.0 (see
../LICENSE-docs).*
