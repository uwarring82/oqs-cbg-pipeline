...
## 2. Sign-convention checklist (signed off 2026-05-12)

Every row below is reconciled against the Companion paper and signed off
by the steward (see §0 and §10). Where the Companion paper differs from
the repository's convention, the difference is recorded as an explicit
**conversion rule**, not silently absorbed. The remaining
implementation-side guards are noted in the row-level status entries;
they do not gate transcription release.
...
| 2.3 | Sign of bath two-point function `C(t, s)` | Eq. (15) verbatim: `D(τ_1^k, s_1^{n−k}) = Tr_E{ B^R(s_1^{n−k}) ∘ B^L(τ_1^k) [ρ_E] } θ_{τ_1^k} θ_{s_1^{n−k}}`. For the simplest case `n=2`, `k=1`: `D(τ_1, s_1) = ⟨B(s_1) B(τ_1)⟩ θ_{τ_1} θ_{s_1}` (right-acting B applied last under the trace, then left-acting B). Note the **Hermiticity** identity Eq. (18): `D*(τ_1^k, s_1^{n−k}) = D(s_1^{n−k}, τ_1^k)`. | Repository n=2 stores `D̄_2(t, s) = C(t − s)` (stationary baseline). `cbg.cumulants.D_bar_2(times)[0, 1]` evaluates to `⟨B(times[0]) B(times[1])⟩` (see [cbg/cumulants.py D_bar_2 path](../cbg/cumulants.py) + scalar leaf [`_D_bar_scalar_from_flat_times`](../cbg/cumulants.py) and the mixed-n=2 witness [tests/test_cumulants.py:test_D_bar_mixed_n2_thermal_matches_pairwise_correlator](../tests/test_cumulants.py)); under the B.0 flattening `times = tau_args + reversed(s_args)`, this returns `⟨B(τ_1) B(s_1)⟩`. | **Derived conversion rule: chain-reversal-and-swap.** Expanding the superop composition in Eq. (15): `B^L(τ_1^k)[ρ] = B(τ_1)...B(τ_k)ρ` (forward order) and `B^R(s_1^{n−k})[X] = X B(s_{n−k})...B(s_1)` (reverse order, due to right-action composition `∘`). Using cyclic trace to move ρ_E to the front yields Companion operator order `(s_{n−k},...,s_1,τ_1,...,τ_k)` inside `Tr{ρ_E ...}`. The repo's `n_point_ordered(tau_args,s_args)` computes `Tr{ρ_E B(t_0)...B(t_{n−1})}` with `times = tau_args + reversed(s_args)`. Setting `tau_args = reversed(s_args_companion)` and `s_args = reversed(tau_args_companion)` gives `times = (s_{n−k},...,s_1,τ_1,...,τ_k)`, matching the Companion trace exactly. The rule is therefore: **`D_companion(τ_1^k, s_1^{n−k}) = n_point_ordered(tau_args=tuple(reversed(s_args)), s_args=tuple(reversed(tau_args)))`**. At `n=2`, `k=1` the swap rule's output is `⟨B(s_1) B(τ_1)⟩`. The naive (no-swap) repo call `n_point_ordered((τ_1,), (s_1,))` would instead return `⟨B(τ_1) B(s_1)⟩`. For Hermitian `B` on a stationary thermal state, the Hermitian-B identity `⟨B(s_1) B(τ_1)⟩ = ⟨B(τ_1) B(s_1)⟩*` connects the two — i.e. the naive output equals the complex conjugate of the swap-rule output. This Hermitian-B identity is the **motivation** for the swap rule (it explains why the naive call disagrees with Eq. (15)), not its **output**: the swap rule produces the correctly-ordered Companion scalar directly, without requiring downstream conjugation. Pure cases (`k=0` or `k=n`) are output-invariant under the swap (the function-argument representation changes, but the flattened `times` array and hence the scalar are the same). | **Closed-by-steward** (derived 2026-05-12). |
...
**Guardian note (historical, retained for audit).** Row 2.8 was the new
sign-convention surface introduced at `n=4`. The rejected single
nested-commutator candidate (see §6) was defeated precisely by
mishandling the 4-point Wick pairing. Row 2.8 is now closed at the
transcription layer (component (a) Draft-complete, component (b)
Closed-by-steward via the row-2.3 derivation). The remaining guard is
implementation-side: a small-grid verification of the direct
Eqs. (69)–(73) formulas under the §4.4 θ-aware combination rule, to be
signed off before Phase B code lands.
...
> is informational only. This artifact is hereby released for Phase B
> consumption, superseding the v0.1.0 pre-release predecessor.
>
> Reviewer: Ulrich Warring  Date: 2026-05-12
>
...
| v0.1.1 | 2026-05-12 | **Post-release docs cleanup (6 findings).** **Medium:** (1) Removed the `---` horizontal rule between the v0.1.0 fix-pass rows and the derivation+release rows so the change-log renders as a single continuous table on GitHub (was breaking the table at the release-critical rows). (2) Filled the §2 internal sign-off block (`Reviewer: Ulrich Warring  Date: 2026-05-12`) to match the §0 frontmatter and §10 countersignature; the v0.1.1 release was countersigned but the §2 sign-off line had been left blank. (3) Rewrote the §10 release sentence from "promoted from `scaffold` to `released`" — which belongs to the predecessor draft lineage — to "released for Phase B consumption, superseding the v0.1.0 pre-release predecessor." **Low:** (4) Updated §2 section title from "must be signed off before Phase B" to "signed off 2026-05-12" with surrounding paragraph rewritten to past-tense. (5) Tightened the final sentence of row 2.3 to distinguish the swap rule's *output* (the correctly-ordered Companion scalar) from the Hermitian-B conjugation *identity* (the relation between the naive no-swap scalar and the swap output that motivates the rule). (6) Re-cast the Guardian note below the §2 sign-off block as historical, pointing only at the remaining implementation-side small-grid guard. No scientific content change; row-2.3 / row-2.8(b) derivation unchanged. | Local steward post-release docs-integrity cleanup. |
| v0.1.1 | 2026-05-13 | **Post-release docs cleanup #2 (1 finding).** **Medium:** Updated row 2.6 status column from "First-pass-Pinned-as-Case-B; **steward to confirm the §5 reconciliation block is complete**" to "**Pinned-as-Case-B** (§5 reconciliation block complete; conclusion identifies Case B and Phase B recommendation = direct Eqs. (69)–(73) evaluation; §2 sign-off recorded 2026-05-12)." The "steward to confirm" wording was stale residue from the v0.1.0 scaffold lineage: the §5 block has carried a populated Case-B conclusion + Phase B recommendation since the 2026-05-12 first-pass fill-in, and §2 was signed off on 2026-05-12 (header line) and countersigned at §10 the same day. The row's §0 cross-reference already reads "row 2.6 is Pinned-as-Case-B (no further action)", which the stale wording contradicted. No scientific content change. | Local steward post-release docs-integrity cleanup #2. |
...
(69)–(78) verbatim into §4, recorded the Λ-inversion reconciliation as
Case B in §5, and strengthened the §7.1 σ_z oracle check. Fix-pass #1
corrected the §7.1 dephasing exactness citation to Letter Eq. (21).
Fix-pass #2 replaced an early §4.4 pointwise pairing-count rule with a
θ-aware procedural rule, split the §0 provenance row to match actual §4
contents, and dropped the §10 arXiv-version-pinned claim so the VoR DOI
is the sole controlling source. The §2.8 fill-in moved row 2.8 from Open
to Draft-complete on the B.0/B.1 vs. Companion `D̄` distinction.
Fix-pass #3 demoted §2.3 and §2.8(b) from "matches Eq. (15)" to a repo
...
swap rule: `D_companion(τ_1^k, s_1^{n−k}) = n_point_ordered(tau_args=
